...
import { accountBalanceFilter, lastSpendFilter, transactionsBalanceFilter } from "../../utils/filter";
   ...
   return (
      <ScrollView className="h-full bg-white">
         ...
         <View>
            ...
            <View className="flex-row justify-between items-center py-2 px-4">
               ...
               <View className="flex-row items-center rounded-lg border border-light-60">
                  {chart.data.map(item => (
                     ...
                     >
                        <View
                           className={`p-2 ${item.value === "LineChart" ? "rounded-tl-lg rounded-bl-lg" : "rounded-tr-lg rounded-br-lg"} ${item.value === chart.isActive && "bg-violet-100"}`}>
                           <SvgIcons
                              icon={item.value}
                              color={`${item.value === chart.isActive ? "#FFF" : "#7F3DFF"}`} />
                        </View>
                     </TouchableWithoutFeedback>
                  ))}
               </View>
            ...
            <Chart version={chart.isActive} data={lastSpendFilter(transactions, 0, transactionButton.isActive.toLowerCase())} />
         </View>
         ...
         <View className="mx-4 mt-3 p-1 h-14 bg-light-60 rounded-[32px] flex-row items-center justify-around">
            {transactionButton.data.map(item => (
               <TouchableWithoutFeedback key={item.id} onPress={() => {
                  setTransactionButton({ ...transactionButton, isActive: item.value });
               }}>
                  <View
                     className={`h-full justify-center items-center w-1/2 rounded-[32px] ${transactionButton.isActive === item.value && "bg-violet-100"}`}>
                     <Text
                        className={`font-medium text-base text-black ${transactionButton.isActive === item.value && "text-light-80"}`}>{item.value}</Text>
                  </View>
               </TouchableWithoutFeedback>
            ))}
         </View>
         ...
         <View className="mt-6">
            ...
            <View className="accounts flex-1 my-4">
               {accountBalanceFilter(userDb.accounts, transactionButton.isActive.toLowerCase(), datex.toUpperCase(), sort).map((account) => (
                  <TouchableNativeFeedback
                     key={account.id}
                  ...
                  >
                     <View
                        className="flex-row justify-between items-center p-4 border-b-[1px] border-[#F7F7F7] ">
                        {/* svg */}
