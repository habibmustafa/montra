   ...
   const handleAmountChange = (text) => {
      ...
      if (regex.test(text)) {
         ...
         if (newAmount === "0." && amount === "0.") {
            ...
            // sadece "." dan sonra en fazla 2 rakam kabul edilir
            let amountArray = newAmount.split(".");
            if (amountArray.length === 2) {
               amountArray[1] = amountArray[1].slice(0, 2);
               newAmount = amountArray.join(".");
            }
            setAmount(newAmount);
            input(newAmount);
         }
   ...
   return (
      <View className="flex-1" style={{ backgroundColor: color }}>
         <View className="input flex-1 px-4" style={{ backgroundColor: color }}>
            ...
            <Text className="text-light-80 font-semibold text-lg mb-1.5 mini:text-base">
               {text}
            </Text>

            <View className="flex-row items-center">
               <Text className="text-[64px] text-light-40 font-medium mini:text-5xl">
                  ₼
               </Text>
               <TextInput
